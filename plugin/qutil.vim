...
function! qutil#CreateCommandQuickfix(lines, name, cmd)
  ...
  " Keeps init#CustomBottomBuffer's bufhidden=wipe: closing the window wipes
  " the buffer, which frees the name for the next list built under it.
...
" Custom quickfixes do not survive being closed, so there is only the native
" list to reopen.
